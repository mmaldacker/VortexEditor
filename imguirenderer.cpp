    ...
    , mCopy(device)
{
  ...
  mSampler =
      Vortex2D::Renderer::SamplerBuilder().Filter(vk::Filter::eLinear).Create(device.Handle());
  ...
  Vortex2D::Renderer::Texture localTexture(device,
                                           mFontTexture.GetWidth(),
                                           mFontTexture.GetHeight(),
                                           mFontTexture.GetFormat(),
                                           VMA_MEMORY_USAGE_CPU_ONLY);

  localTexture.CopyFrom(pixels);
  device.Execute(
      [&](vk::CommandBuffer commandBuffer) { mFontTexture.CopyFrom(commandBuffer, localTexture); });
...
void ImGuiRenderer::Update()
{
  ...
  for (int n = 0; n < data->CmdListsCount; n++)
  {
    ...
    mLocalVertexBuffer.CopyFrom(vtxOffset * sizeof(ImDrawVert),
                                cmd_list->VtxBuffer.Data,
                                cmd_list->VtxBuffer.Size * sizeof(ImDrawVert));
    mLocalIndexBuffer.CopyFrom(idxOffet * sizeof(ImDrawIdx),
...
void ImGuiRenderer::Draw(vk::CommandBuffer commandBuffer,
                         const Vortex2D::Renderer::RenderState& renderState)
{
  auto* data = ImGui::GetDrawData();
  if (data == nullptr)
  {
  ...
  commandBuffer.pushConstants(mDescriptorSet.pipelineLayout,
                              ...
                              0,
                              vk::ArrayProxy<const glm::vec2>{scale, translate});
  ...
  for (int n = 0; n < data->CmdListsCount; n++)
  {
    ...
    for (int i = 0; i < cmdList->CmdBuffer.Size; i++)
    {
      ...
      else
      {
        ...
        vk::Extent2D extent((uint32_t)(pcmd->ClipRect.z - pcmd->ClipRect.x),
